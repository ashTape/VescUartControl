...
void SetSerialPort(VescIO *_serialPort) { vesc_io = _serialPort; }

void SetDebugSerialPort(DEBUG_SERIAL_CLASS *_debugSerialPort) { debugSerialPort = _debugSerialPort; }

int ReceiveUartMessage(uint8_t *payloadReceived, VescIO *_vescserialPort) {
  // Messages <= 255 start with 2. 2nd byte is length
  // Messages >255 start with 3. 2nd and 3rd byte is length combined with 1st
  // >>8 and then &0xFF
  ...
  counter +=
      _vescserialPort->readBytes(messageReceived, 2);  // get payload size

  switch (messageReceived[0]) {
    case 2:
      lenPayload = messageReceived[1];
      endMessage =
          lenPayload + 5;  // Payload size + 2 for sice + 3 for SRC and End.

      break;
    case 3:  // must be careful that rx buffer does not overflow.
      // counter +=    _vescserialPort->readBytes(&messageReceived[counter], 1);
      // //
      // get additional payload size byte
      // lenPayload = (messageReceived[1]<<8) + messageReceived[2]; // is this
      // the
      // right endian?
      // endMessage = lenPayload + 5; //Payload size + 2 for sice + 3 for SRC
      // and
      // End.
  ...
  counter += _vescserialPort->readBytes(
      &messageReceived[counter],
      endMessage - counter);  // get payload, crc, endbyte
...
bool UnpackPayload(uint8_t *message, int lenMes, uint8_t *payload, int lenPay) {
  ...
  if (debugSerialPort != NULL) {
    debugSerialPort->print("SRC received: ");
    debugSerialPort->println(crcMessage);
  }  // DEBUG
  ...
  if (debugSerialPort != NULL) {
    debugSerialPort->print("SRC calc: ");
    debugSerialPort->println(crcPayload);
  }
  if (crcPayload == crcMessage) {
    if (debugSerialPort != NULL) {
      ...
      // debugSerialPort->print("Payload :            "); SerialPrint(payload,
      // message[1] - 1); debugSerialPort->println();
    }  // DEBUG
    return true;
  } else {
    return false;
...
int PackSendPayload(uint8_t *payload, int lenPay, VescIO *_vescserialPort) {
  ...
  if (debugSerialPort != NULL) {
    debugSerialPort->print("UART package send: ");
...
bool VescUartGetValue(bldcMeasure &values, VescIO *_vescserialPort) {
  ...
  if (lenPayload > 1) {
    bool read = ProcessReadPacket(payload, values,
                                  lenPayload);  // returns true if sucessful
    return read;
  } else {
    return false;
  }
...
void VescUartSetCurrent(float current) { VescUartSetCurrent(current, 0); }
...
void VescUartSetDuty(float duty) { VescUartSetDuty(duty, vesc_io); }
...
void VescUartSetRPM(float rpm) { VescUartSetRPM(rpm, vesc_io); }
...
void SerialPrint(uint8_t *data, int len) {
  debugSerialPort->print("Data to display: ");
  debugSerialPort->println(len);
...
void SerialPrint(const struct bldcMeasure &values,
  ...
  print_serialPort->print("tempFetFiltered: ");
  print_serialPort->println(values.tempFetFiltered);
  print_serialPort->print("tempMotorFiltered:");
  print_serialPort->println(values.tempMotorFiltered);
  print_serialPort->print("avgMotorCurrent: ");
  print_serialPort->println(values.avgMotorCurrent);
  print_serialPort->print("avgInputCurrent: ");
  print_serialPort->println(values.avgInputCurrent);
  print_serialPort->print("avgId: ");
  print_serialPort->println(values.avgId);
  print_serialPort->print("avgIq: ");
  print_serialPort->println(values.avgIq);
  print_serialPort->print("dutyNow: ");
  print_serialPort->println(values.dutyNow);
  print_serialPort->print("rpm: ");
  print_serialPort->println(values.rpm);
  print_serialPort->print("inpVoltage: ");
  print_serialPort->println(values.inpVoltage);
  print_serialPort->print("ampHours: ");
  print_serialPort->println(values.ampHours);
  print_serialPort->print("ampHoursCharged: ");
  print_serialPort->println(values.ampHoursCharged);
  print_serialPort->print("tachometer: ");
  print_serialPort->println(values.tachometer);
  print_serialPort->print("tachometerAbs: ");
  print_serialPort->println(values.tachometerAbs);
  print_serialPort->print("faultCode: ");
  print_serialPort->println(values.faultCode);
}
